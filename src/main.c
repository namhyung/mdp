...
void usage() {
    ...
    fprintf(stderr, "%s", "  -w, --warning     show warning on small terminal and exit\n");
...
int main(int argc, char *argv[]) {
    ...
    int warning = 0;   // 0:don't show; 1:show warning on small terminal
    ...
    int opt, debug = 0;
    while ((opt = getopt_long(argc, argv, ":defhitvsxw", longopts, NULL)) != -1) {
        switch(opt) {
    ...
    do {
        ...
        reload = ncurses_display(deck, notrans, nofade, invert, reload, noreload, slidenum, warning);
